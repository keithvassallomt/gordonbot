...
type startedMsg struct{ what string; ok bool; err error }
...
func mediamtxRunning(httpPort int, cmdPath string) bool {
    if haveBin("pgrep") {
        if exec.Command("bash", "-lc", "pgrep -x mediamtx").Run() == nil { return true }
        if cmdPath != "" {
            cmd := fmt.Sprintf("pgrep -f %q", cmdPath)
            if exec.Command("bash", "-lc", cmd).Run() == nil { return true }
        }
    }
    if haveBin("lsof") {
...
func startProcess(name string, script string, workdir string) (*proc, error) {
    ...
    p.setAlive(true)
    procsMu.Lock(); procs[name] = p; procsMu.Unlock()

    // Stream stdout
    go func() {
        scanner := bufio.NewScanner(stdout)
        for scanner.Scan() {
            if program != nil { program.Send(logMsg{fmt.Sprintf("[%s] %s", name, scanner.Text())}) }
        }
    }()
    // Stream stderr
    go func() {
    ...
    go func() {
        _ = cmd.Wait()
        p.setAlive(false)
        if program != nil { program.Send(procExitMsg{name: name}) }
    }()
    return p, nil
}
...
func stopProcess(p *proc) {
    if p == nil { return }
    p.mu.Lock(); cmd := p.cmd; cancel := p.cancel; p.mu.Unlock()
    if cmd == nil { return }
...
func stopAll() {
    ...
    for k, v := range procs { m[k] = v }
    procsMu.Unlock()
    for _, p := range m { stopProcess(p) }
}
...
func (m model) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
    ...
    case tea.KeyMsg:
        if m.showLevelModal {
            ...
            case "esc":
                ...
                return m, nil
            case "up", "k":
                if m.modalIndex > 0 { m.modalIndex-- }
                return m, nil
            case "down", "j":
        ...
        case "q", "ctrl+c":
            return m, tea.Quit
        case "r":
            m.status = "restarting"; m.starting = true
    ...
    case telemetryMsg:
        m.cpuTemp = msg.cpu
        m.battPct = msg.pct
        if msg.state != "" { m.battState = msg.state }
...
func restartFrontendCmd(c cfg) tea.Cmd {
    return func() tea.Msg {
        ...
        procsMu.Lock(); old := procs["frontend"]; procsMu.Unlock(); stopProcess(old)
        killPort(c.frontendPort)
        feCmd := "stdbuf -oL -eL npm run dev -- --host --port " + strconv.Itoa(c.frontendPort)
...
func headerLines(m model) string {
    ...
    // Telemetry
    cpu := "—"
    if m.cpuTemp != nil { cpu = fmt.Sprintf("%.1f°C", *m.cpuTemp) }
    bp := "—"
    if m.battPct != nil { bp = fmt.Sprintf("%.0f%%", *m.battPct) }
...
func (m *model) filteredLines() []string {
    if m.levelThreshold <= 10 { return m.logs }
    out := make([]string, 0, len(m.logs))
    for _, ln := range m.logs {
        if lineLevel(ln) >= m.levelThreshold { out = append(out, ln) }
    }
    return out
}
...
func (m model) levelModalView() string {
    ...
    for i, name := range names {
        cursor := "  "
        if i == m.modalIndex { cursor = "> " }
        line := fmt.Sprintf("%s%d. %s", cursor, i+1, name)
        if i == m.modalIndex { line = lipgloss.NewStyle().Bold(true).Render(line) }
        fmt.Fprintln(&b, line)
    }
...
func fetchBattery(client *http.Client, c cfg) (*float64, string) {
    ...
    if err := json.NewDecoder(resp.Body).Decode(&b); err != nil { return nil, "" }
    var st string
    if b.Charging != nil { st = *b.Charging }
    return &b.Percent, st
}
